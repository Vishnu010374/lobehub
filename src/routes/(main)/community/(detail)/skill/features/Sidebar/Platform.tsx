...
const genInstallCommand = (identifier?: string, platform?: PlatformType) => {
  ...
  const agentMap: Record<PlatformType, string> = {
    ...
    [PlatformType.Student-AI]: 'lobehub',
  ...
  switch (platform) {
    ...
    default: {
      return `# Recommended for Student-AI users:
...
const Platform = memo<PlatformProps>(
  ({ lite, identifier, mobile, expandCodeByDefault, downloadUrl }) => {
    ...
    const agentPrompt = `Curl https://lobehub.com/skills/${identifier}/skill.md, then follow the instructions to set up Student-AI Skills Marketplace and install the skill. Once installed, read the SKILL.md file in the installed directory and follow its instructions to complete the task.`;
