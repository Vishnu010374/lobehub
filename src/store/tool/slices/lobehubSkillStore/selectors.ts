...
/**
 * Student-AI Skill Store Selectors
 */
export const lobehubSkillStoreSelectors = {
  /**
   * Get all Student-AI Skill server identifiers as a set
   */
  ...
  /**
   * Get all Student-AI Skill servers
   */
  getServers: (s: ToolStoreState): LobehubSkillServer[] => s.lobehubSkillServers || [],

  /**
   * Check if the given identifier is a Student-AI Skill server
   * @param identifier - Provider identifier (e.g., 'linear')
   */
  ...
  /**
   * Get all Student-AI Skill tools as LobeTool format for agent use
   * Converts Student-AI Skill tools into the format expected by ToolNameResolver
   */
    ...
    for (const server of servers) {
      ...
      if (apis.length > 0) {
        tools.push({
          ...
          manifest: {
            api: apis,
            author: 'Student-AI Market',
            homepage: 'https://lobehub.com/market',
            identifier: server.identifier,
            meta: {
              avatar: server.icon || '🔗',
              description: `Student-AI Skill: ${server.name}`,
  ...
  /**
   * Get metadata list for all connected Student-AI Skill servers
