...
show_message() {
    ...
    case $key in
        ...
        tips_already_installed)
            case $LANGUAGE in
                zh_CN)
                    echo "检测到您已经运行过 Student-AI，本安装程序只能完成初始化配置，并不能重复安装。如果你需要重新安装，请删除 data 和 s3_data 文件夹。"
                ;;
                *)
                    echo "It is detected that you have run Student-AI. This installation program can only complete the initialization configuration and cannot be reinstalled. If you need to reinstall, please delete the data and s3_data folders."
                ;;
            esac
        ;;
        tips_run_command)
            case $LANGUAGE in
                zh_CN)
                    echo "您已经完成了所有配置。请运行以下命令启动 Student-AI 尝试启动："
                ;;
                *)
                    echo "You have completed all configurations. Please run this command to start Student-AI:"
                ;;
            esac
        ;;
        ...
        tips_if_run_normally)
            case $LANGUAGE in
                zh_CN)
                    echo "如果一切运行正常，你可以使用以下指令在 daemon 模式下启动 Student-AI:"
                ;;
                *)
                    echo "If everything runs normally, you can use the following command to start Student-AI in daemon mode:"
                ;;
            esac
        ;;
        tips_regen_jwks)
        ...
        ask_deploy_mode)
            case $LANGUAGE in
                zh_CN)
                    echo "请选择部署模式："
                    echo "(0) 域名模式（访问时无需指明端口），需要使用反向代理服务 Student-AI, RustFS，并分别分配一个域名；"
                ...
                *)
                    ...
                    echo "(0) Domain mode (no need to specify the port when accessing), you need to use the reverse proxy service Student-AI, RustFS, and assign a domain name respectively;"
...
section_configurate_host() {
    ...
    case $DEPLOY_MODE in
        0)
            DEPLOY_MODE="domain"
            echo "Student-AI" $(show_message "ask_domain" "example.com")
            ask "(example.com)"
        ...
        1)
            DEPLOY_MODE="ip"
            ask $(printf "%s%s" "Student-AI" $(show_message "ask_host")) "$HOST" $(printf "%s" $(show_message "tips_auto_detected"))
            LOBE_HOST="$ask_result"
