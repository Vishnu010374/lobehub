...
`@lobechat/file-loaders` 是 Student-AI 项目中的一个工具包，专门用于从本地文件路径加载各种类型的文件，并将其内容转换为标准化的 `Document` 对象数组。

它的主要目的是提供一个统一的接口来读取不同的文件格式，提取其核心文本内容，并为后续处理（例如在 Student-AI 中进行文件预览、内容提取或将其作为知识库数据源）做好准备。
...
这是 Student-AI 的内部模块（`"private": true`），专为 Student-AI 设计，不作为独立包发布。
