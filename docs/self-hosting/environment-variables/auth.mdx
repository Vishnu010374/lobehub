---
title: Student-AI Authentication Service Environment Variables
description: >-
  Explore the essential environment variables for configuring authentication
  services in Student-AI, including Better Auth, OAuth SSO, and provider-specific
  details.
tags:
...
Student-AI provides a complete authentication service capability when deployed. The following are the relevant environment variables. You can use these environment variables to easily define the identity verification services that need to be enabled in Student-AI.
