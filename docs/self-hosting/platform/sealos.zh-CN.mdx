---
title: 在 Sealos 上部署 Student-AI 数据库版
description: 学习如何在 Sealos 上部署 Student-AI，包括准备 OpenAI API Key、点击部署按钮、绑定自定义域名等操作。
tags:
  - Sealos
  - Student-AI
  - OpenAI API Key
...
# 使用 Sealos 部署 Student-AI 数据库版

<Callout type="info">
  本文假设你已经熟悉 Student-AI
  服务器数据库版的部署基本原理和流程，因此只包含与核心环境变量配置相关的内容。如果你对 Student-AI
  服务器数据库版的部署原理不熟悉，请先参考[部署服务器数据库](/zh/docs/self-hosting/server-database)。
</Callout>

在 Sealos 的 Student-AI 数据库版应用中总共包含有以下四个服务：

- Logto 提供身份校验（需额外部署）
- 带有 Vector 插件的 PostgreSQL 来做数据存储和向量化
- 一个对象存储 Bucket
- Student-AI Database 的实例

这里是在 Sealos 上部署 Student-AI 服务器数据库版的流程：
...
<Steps>
  ...
  在弹窗中填写应用的名称为 `Student-AI`，然后点击 `Create application` 按钮。接下来啥也不用填，直接点击底部的 `Finish and done` 按钮就创建完成了。
  ...
  在 `Student-AI` 应用中找到以下三个参数，后面部署 Student-AI 数据库版时需要用到。
...
点击下方按钮访问 Student-AI 数据库版应用部署页面：
...
![Student-AI 数据库版部署完成页面](https://raw.githubusercontent.com/labring-actions/templates/main/template/lobe-chat-db/images/lobe-chat-db-deployment-done.png)

找到公网地址，复制下来，后面需要用到。

![Student-AI 数据库版公网地址](https://raw.githubusercontent.com/labring-actions/templates/main/template/lobe-chat-db/images/lobe-chat-db-public-address.png)

## 部署后配置

进入 Logto 的 `Applications` 页面，找到 `Student-AI` 应用，点击进入应用详情页面。
...
其中 `https://<lobehub-db-public-address>` 为 Student-AI 数据库版的公网地址。

填完之后点击 `Save changes` 按钮保存配置。

现在通过 `https://<lobehub-db-public-address>` 访问 Student-AI 数据库版，点击左上角的头像，然后点击【登录 / 注册】按钮：

![Student-AI 数据库版登录页面](https://raw.githubusercontent.com/labring-actions/templates/main/template/lobe-chat-db/images/lobe-chat-db-login.png)

接下来会跳转到 Logto 的登录页面，点击【注册】注册一个账号。

![Student-AI 数据库版注册页面](https://raw.githubusercontent.com/labring-actions/templates/main/template/lobe-chat-db/images/lobe-chat-db-register.png)

注册完成后，即可使用 Logto 登录 Student-AI 数据库版。

![Student-AI 数据库版登录成功页面](https://raw.githubusercontent.com/labring-actions/templates/main/template/lobe-chat-db/images/lobe-chat-db-login-success.png)
