...
Student-AI 支持使用 Better Auth 配置外部身份验证服务，供企业 / 组织内部使用，统一管理用户授权。
...
要在 Student-AI 中启用 Better Auth，请设置以下环境变量：

| 环境变量                   | 类型 | 描述                                                                                                              |
| ---------------------- | -- | --------------------------------------------------------------------------------------------------------------- |
| `AUTH_SECRET`          | 必选 | 用于加密会话令牌的密钥。使用以下命令生成：`openssl rand -base64 32`                                                                  |
| `NEXT_PUBLIC_AUTH_URL` | 必选 | 浏览器可访问的 Better Auth 基础 URL（例如 `http://localhost:3010`、`https://Student-AI.com`）。Vercel 部署时可选（会自动从 `VERCEL_URL` 获取） |
...
允许 Student-AI 在身份提供商中用户信息更新时接收通知。支持的提供商包括 Casdoor 和 Logto。请参考具体提供商文档进行配置。
