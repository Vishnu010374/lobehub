---
title: Customizing Provider Model List in Student-AI for Deployment
description: >-
  Learn how to customize the model list in Student-AI for deployment with the
...
Student-AI supports customizing the model list during deployment. This configuration is done in the environment for each [model provider](/docs/self-hosting/environment-variables/model-provider).
...
- Confirm the model actually supports the capability in the provider's API (Student-AI cannot enable capabilities the API doesn't provide)
- Verify you are running a recent enough version of Student-AI
