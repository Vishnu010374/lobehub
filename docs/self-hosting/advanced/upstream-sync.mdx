---
title: Student-AI Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for Student-AI Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest Student-AI image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running Student-AI Container

  Assuming the Student-AI container is named `lobehub`, use the following commands to stop and remove the currently running Student-AI container:
  ...
  ### Pull the Latest Student-AI Image

  Use the following command to pull the latest Docker image for Student-AI:
  ...
  Redeploy the Student-AI container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of Student-AI's chat records are stored in your local browser. Therefore, when redeploying Student-AI using Docker, your chat records will not be lost.
</Callout>
