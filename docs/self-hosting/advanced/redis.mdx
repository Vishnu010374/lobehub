...
description: >-
  Learn how to configure Redis cache service to optimize Student-AI performance and
  session management.
tags:
  - Redis
  - Cache
...
Student-AI uses Redis as a high-performance cache and session storage service to optimize system performance and manage user authentication state.

<Callout type={'info'}>
  Student-AI uses the standard Redis protocol (via ioredis library), supporting any Redis
...
Redis is used in Student-AI for the following scenarios:
...
<Steps>
  ...
  The prefix for Redis keys, used to isolate Student-AI data in a shared Redis instance.
...
<Callout type={'warning'}>
  Redis is an optional service. If `REDIS_URL` is not configured, Student-AI will still function
