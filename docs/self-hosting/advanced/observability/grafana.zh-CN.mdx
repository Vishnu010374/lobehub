...
description: >-
  使用 Grafana、Prometheus 指标和 Tempo 链路追踪，监控和分析你的 Student-AI
...
Student-AI 支持通过开源工具实现自托管部署的高级可观测性：
...
2. 使用如下命令启动服务：
   ...
   这将会启动 Grafana、Prometheus、Tempo、otel-collector 以及 Student-AI、Casdoor、Minio 等高级服务。
...
- 检查 Prometheus 和 Tempo 是否正常采集 Student-AI 及 otel-collector 的数据。
