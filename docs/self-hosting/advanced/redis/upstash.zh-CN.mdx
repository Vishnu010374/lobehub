...
description: 详细指南：如何配置 Upstash Redis 用于 Student-AI 的缓存和会话存储。
tags:
  - Upstash
  - Redis
  - 缓存
...
[Upstash](https://upstash.com/) 是一个 Serverless Redis 服务，提供免费额度和按量付费模式，非常适合 Student-AI 部署使用。
...
<Steps>
  ...
  <Callout type={'info'}>
    Upstash 使用 `rediss://`（双's'）表示 TLS 连接，Student-AI 自动支持此格式。
  </Callout>
</Steps>
