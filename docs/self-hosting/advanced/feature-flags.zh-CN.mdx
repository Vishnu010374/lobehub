---
title: Student-AI 特性标志环境变量配置指南
description: 了解如何使用环境变量自定义 Student-AI 的特性标志，包括控制否启用某个功能、或者根据需要对特定用户群体或环境启用或禁用功能。
tags:
  - Student-AI
  - 环境变量
  - 配置指南
...
除了基础的环境变量配置外，Student-AI 还提供了一些特性标志（Feature Flags），用于控制是否全局启用某个功能，或者根据需要对特定用户群体或环境启用或禁用功能。
...
除了上述 `FEATURE_FLAGS` 体系外，Student-AI 还提供了一批独立的环境变量，用于控制依赖外部基础设施的特定功能。这些是独立变量（不属于 `FEATURE_FLAGS`）：
