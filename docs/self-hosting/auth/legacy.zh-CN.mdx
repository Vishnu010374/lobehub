...
Student-AI 与 Clerk 做了深度集成，能够为用户提供安全、便捷的登录和注册体验。

在 Student-AI 的环境变量中设置 `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` 和 `CLERK_SECRET_KEY`，即可开启和使用 Clerk。
...
在使用 NextAuth 之前，请先在 Student-AI 的环境变量中设置以下变量：
