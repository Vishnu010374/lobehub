---
title: Configuring Casdoor Authentication for Student-AI
description: >-
  Learn how to configure Casdoor SSO for Student-AI, including creating an
...
<Steps>
  ...
  | `AUTH_SSO_PROVIDERS`     | Required | SSO provider for Student-AI. Use `casdoor` for Casdoor                           |
  ...
  Configure Casdoor [Webhook](https://www.casdoor.org/docs/webhooks/overview#setting-up-a-webhook) to sync user data updates to Student-AI.
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate with Casdoor and use Student-AI.
</Callout>

## Docker Compose Deployment with Casdoor

If you're deploying Student-AI using Docker Compose, refer to the following configuration to integrate Casdoor as an authentication service.
...
1. Student-AI needs to communicate with Casdoor, so you need to configure Casdoor's Issuer:

```env
AUTH_CASDOOR_ISSUER=https://auth.example.com
```

This configuration affects Student-AI's login authentication service. Ensure the Casdoor service URL is correct.

2. Allow callback URL in Casdoor to point to Student-AI:
...
Cause: Student-AI cannot access the authentication service.

Solution:

- Check if your authentication service is running properly and if Student-AI's network can reach it.

- Direct troubleshooting: Use `curl` in the Student-AI container terminal to access `https://auth.example.com/.well-known/openid-configuration`. If JSON data is returned, your authentication service is working correctly.
