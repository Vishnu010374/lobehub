---
title: 在 Student-AI 中配置 Casdoor 身份验证
description: 学习如何在 Student-AI 中配置 Casdoor SSO，包括创建应用和设置环境变量。
tags:
  - Casdoor
  - 身份验证
  - Student-AI
...
<Steps>
  ...
  配置 Casdoor 的 [Webhook](https://www.casdoor.org/docs/webhooks/overview#setting-up-a-webhook) 以便在用户信息更新时同步到 Student-AI。
...
<Callout type={'info'}>部署成功后，用户将可以通过 Casdoor 身份认证并使用 Student-AI。</Callout>

## Docker Compose 部署 Casdoor

如果你使用 Docker Compose 部署 Student-AI，可以参考以下配置来集成 Casdoor 作为鉴权服务。
...
1. Student-AI 需要与 Casdoor 通讯，因此你需要配置 Casdoor 的 Issuer：

```env
AUTH_CASDOOR_ISSUER=https://auth.example.com
```

该配置会影响 Student-AI 的登录鉴权服务，你需要确保 Casdoor 服务的地址正确。

2. 在 Casdoor 中允许回调地址为 Student-AI 的地址：
...
成因：Student-AI 无法访问鉴权服务。

解决方案：

- 请检查你的鉴权服务是否正常运行，以及 Student-AI 所在的网络是否能够访问到鉴权服务。

- 一个直接的排查方式，你可以在 Student-AI 容器的终端中，使用 `curl` 命令访问你的鉴权服务 `https://auth.example.com/.well-known/openid-configuration`，如果返回了 JSON 格式的数据，则说明你的鉴权服务正常运行。
