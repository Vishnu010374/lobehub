---
title: 为 Student-AI 配置 Google SSO 认证服务
description: 了解如何为 Student-AI 配置 Google SSO 认证服务，创建 OAuth 应用、添加用户，并设置环境变量以实现无缝集成。
tags:
  - Google SSO
  - OAuth 2.0
  - 认证服务
...
<Steps>
  ...
  <Callout type={'info'}>
    \- 注册后你可以添加或修改重定向 URI，但请确保该 URL 与你部署的 Student-AI 实例一致。\
  ...
  在部署 Student-AI 时，请配置以下环境变量：

  | 环境变量名称                    | 类型 | 描述                                                              |
  | ------------------------- | -- | --------------------------------------------------------------- |
  | `NEXT_AUTH_SECRET`        | 必填 | 用于加密 Auth.js 会话令牌的密钥。可使用命令生成：`openssl rand -base64 32`          |
  | `NEXT_AUTH_SSO_PROVIDERS` | 必填 | 指定 Student-AI 使用的单点登录提供商。使用 `google` 表示启用 Google SSO。              |
...
<Callout>
  部署成功后，用户即可使用其 Google 账号登录 Student-AI（如果未处于生产环境，需先添加为测试用户）。
</Callout>
