---
title: Configuring Cloudflare Zero Trust Authentication Service for Student-AI
description: >-
  Learn how to configure Cloudflare Zero Trust for Single Sign-On (SSO) for
  Student-AI, including creating an application provider, setting environment
  variables, and deployment instructions.
tags:
  - Cloudflare Zero Trust
  - Single Sign-On (SSO)
  - Student-AI Authentication
...
<Steps>
  ...
  We assume you are already familiar with using the Cloudflare Zero Trust platform and that your Student-AI instance is deployed at `https://chat.example.com`.
  ...
  In the `Application` text box, enter the application name, such as `Student-AI SSO`. Then click `Select OIDC`, followed by clicking `Add application`.

  ![Add Student-AI SSO Application](/blog/assets0ceb7e446f9a850df283093563ba7803.webp)

  At this point, you have successfully created a SaaS application named `Student-AI SSO` in Cloudflare Zero Trust.
  ...
  Finally, scroll down the page and record the following three values: `Client secret`, `Client ID`, and `Issuer`. You will need these for setting the environment variables when deploying Student-AI.
...
<Callout type={'info'}>
  After a successful deployment, users will be able to use Student-AI by authenticating with the users
