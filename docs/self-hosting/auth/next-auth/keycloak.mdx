---
title: Configuring Keycloak Authentication Service in Student-AI
description: >-
  Learn how to configure the Keycloak authentication service in Student-AI,
...
# Configuring Keycloak Authentication Service in Student-AI
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with Student-AI via Docker
...
- Your Student-AI database version IP/port is `http://Student-AI_IP:3210`.
- Your privately deployed Keycloak domain is `http://KEYCLOAK_IP:8080`.

If you deploy using a public network, this guide assumes:

- Your Student-AI database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "Student-AI", then click "Create"

  2. Create a Client

     - Select "Clients" from the left menu, then click "Create client"
     - Fill in the following information:
       - Client ID: `Student-AI`
     ...
     - On the "Login settings" page:
       - Valid redirect URIs:
         ...
         - Local network IP deployment: `http://Student-AI_IP:3210/api/auth/callback/keycloak`
         - Public environment: `https://lobe.example.com/api/auth/callback/keycloak`
       - Web origins: Add your Student-AI domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the Student-AI environment variables.

  Configure the Student-AI environment variable `AUTH_KEYCLOAK_ISSUER` as:

  - `http://localhost:8080/realms/Student-AI` for local development environment
  - `http://KEYCLOAK_IP:8080/realms/Student-AI` for privately deployed Keycloak on a local network
  - `https://lobe-auth-api.example.com/realms/Student-AI` for Keycloak deployed in a public environment

  When deploying Student-AI, you need to configure the following environment variables:
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for Student-AI. For Keycloak, fill in `keycloak`.                                                                                                        |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate through Keycloak and use Student-AI.
</Callout>
