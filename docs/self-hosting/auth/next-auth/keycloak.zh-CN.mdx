---
title: 在 Student-AI 中配置 Keycloak 身份验证服务
description: 学习如何在 Student-AI 中配置 Keycloak 身份验证服务，包括部署、创建、设置权限和环境变量。
tags:
  - Keycloak 身份验证
  - 环境变量配置
  - 单点登录
...
<Callout type={'tip'}>
  若你想要私有部署 Keycloak，我们建议你将之与 Student-AI 一同使用 Docker Compose
  部署，这样可以更方便地管理服务。
</Callout>
...
- 你的 Student-AI IP / 端口为 `http://Student-AI_IP:3210`。
- 你私有部署 Keycloak，其域名为 `http://KEYCLOAK_IP:8080`。

若你使用公网部署，下文假设：

- 你的 Student-AI 域名为 `https://lobe.example.com`。
...
<Steps>
  ...
  1. 创建新领域（Realm）
     ...
     - 输入名称，例如 "Student-AI"，然后点击 "Create"
  ...
  2. 创建客户端（Client）
     ...
     - 在 "Login settings" 页面：
       - Valid redirect URIs:
         ...
         - 局域网 IP 部署：`http://Student-AI_IP:3210/api/auth/callback/keycloak`
         - 公网环境：`https://lobe.example.com/api/auth/callback/keycloak`
       - Web origins: 添加你的 Student-AI 域名或 IP
       - 点击 "Save"
  ...
  将获取到的客户端 ID 和客户端密钥，设为 Student-AI 环境变量中的 `AUTH_KEYCLOAK_ID` 和 `AUTH_KEYCLOAK_SECRET`。

  配置 Student-AI 环境变量中 `AUTH_KEYCLOAK_ISSUER` 为：

  - `http://localhost:8080/realms/Student-AI`，若你是本地开发环境
  - `http://KEYCLOAK_IP:8080/realms/Student-AI`，若你是局域网私有部署的 Keycloak
  - `https://lobe-auth-api.example.com/realms/Student-AI`，若你是公网环境部署的 Keycloak

  在部署 Student-AI 时，你需要配置以下环境变量：
...
<Callout type={'info'}>部署成功后，用户将可以通过 Keycloak 身份认证并使用 Student-AI。</Callout>
