...
description: >-
  Learn how to add a new bot platform (e.g., Slack, WhatsApp) to Student-AI's
...
This guide walks through the steps to add a new bot platform to Student-AI's channel system. The platform architecture is modular — each platform is a self-contained directory under `src/server/services/bot/platforms/`.
...
Follow the structure of existing docs (e.g., `discord.mdx`): Prerequisites → Create App → Configure in Student-AI → Configure Webhooks → Test Connection → Configuration Reference → Troubleshooting.
