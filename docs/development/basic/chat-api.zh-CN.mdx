...
description: 深入了解 Student-AI Chat API 的前后端交互实现逻辑和核心组件。
tags:
  - Chat API
  - 前后端交互
  - Model Runtime
...
本文档说明了 Student-AI Chat API 在前后端交互中的实现逻辑，
包括事件序列和涉及的核心组件。
...
`call_tool` 指令。Student-AI 支持三种工具类型：
...
Model Runtime（`packages/model-runtime/`）是 Student-AI 中
与 LLM 模型提供商交互的核心抽象层，
